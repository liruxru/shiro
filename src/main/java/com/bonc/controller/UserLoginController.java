...
import com.bonc.utils.UserManager;
import com.bonc.utils.VerifyCodeUtils;
...
public class UserLoginController extends BaseController {
	...
	public Map<String,Object> subRegister(String vcode,UUser entity){
		...
		logger.debug( "注册插入完毕！{}", JSONObject.fromObject(entity).toString());
		entity = TokenManager.login(entity, Boolean.TRUE);
		logger.debug( "注册后，登录完毕！{}", JSONObject.fromObject(entity).toString());
	...
	public Map<String,Object> submitLogin(UUser entity,Boolean rememberMe,HttpServletRequest request){
		...
		try {
			 ...
			 */
			request.getSession().setAttribute("token", entity);
			logger.debug( "获取登录之前的URL:[%s]",url);
			//如果登录之前没有地址，那么就跳转到首页。
	...
	public Map<String,Object> logout(){
		try {
			...
			logger.error( "退出出现错误，{}。", e.getMessage());
		}
		return resultMap;
	}
}
