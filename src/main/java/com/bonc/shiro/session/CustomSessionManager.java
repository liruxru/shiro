package com.bonc.shiro.session;
...
import com.bonc.pojo.UserOnlineBo;
import com.bonc.shiro.session.dao.CustomShiroSessionDAO;
...
public class CustomSessionManager {
	...
	public UserOnlineBo getSession(String sessionId) {
		Session session = customShiroSessionDAO.doReadSession(sessionId);
		UserOnlineBo bo = getSessionBo(session);
		return bo;
	}
	...
	public Map<String, Object> changeSessionStatus(Boolean status,
		...
		try {
			...
			for (String id : sessionIdArray) {
				Session session = this.customShiroSessionDAO.doReadSession(id);
	...
	public void forbidUserById(Long id, Long status) {
		...
		for(UserOnlineBo bo : getAllUser()){
			...
			if(userId.equals(id)){
				//获取用户Session
				Session session = this.customShiroSessionDAO.doReadSession(bo.getSessionId());
				//标记用户Session
				SessionStatus sessionStatus = (SessionStatus) session.getAttribute(SESSION_STATUS);
